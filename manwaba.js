    ...
    comic = {
        ...
        loadEp: async (comicId, epId) => {
            ...
            for (const img of imgElements) {
                ...
                if (src && 
                    ...
                    !src.includes("blank") &&
                    !src.includes("imagecover3") &&  // 占位图
                    !src.includes("mwmissing") &&    // 缺失图
                    !src.includes("blob:") &&        // blob占位
                    !src.includes("imagecover") &&   // 其他占位
                    !src.includes("default")) {      // 默认图
                    images.push(src)
                }
            }
            doc.dispose()
        ...
        onImageLoad: (url, comicId, epId) => {
            ...
            if (!url.startsWith('http://') && !url.startsWith('https://')) {
                fullUrl = this.toAbsoluteUrl(url)
            }
